...
publishMods {
	...
	if (shouldAnnounceDiscord) {
		...
		discord("publishDiscordBody") {
			...
			if (!isDev) {
				content = changelog.map { "# [BabySulfurCubeBlocks version `$version` is out!](https://modrinth.com/mod/babycubeblocks/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n"  }
			}
			else {
				content = changelog.map { "# [BabySulfurCubeBlocks version `$version` is out!](https://modrinth.com/mod/babycubeblocks-dev/versions)\n" +
					"### Changelog:\n" +
					"```\n$it```\n" }
			}
